...
| #383 — pywebview | Stale dependency branch and major desktop runtime change. `requirements-desktop.txt` is rendered from the canonical TheHub federation template. | **Closed and deferred to a federation-wide TheHub template vector.** The MoneySweep-only edit was removed from #431 after the template-drift gate correctly rejected it. |
...
- **#431 — dependency refresh:** reconstructs the openpyxl, Ruff, mypy, and PyArrow intentions from current `main` in five reviewable files; pywebview is explicitly excluded as a canonical-template concern.
...
- Stale intent was either reconstructed from current `main`, deferred to its canonical owner, or formally superseded; no 1,000-file legacy branch was rebased wholesale.

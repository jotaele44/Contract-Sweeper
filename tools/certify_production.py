...
def _preflight(root: Path) -> dict[str, Any]:
    ...
    return run_pipeline_preflight(root, logger, strict=True, write_report=False)
...
def build_report(
    *,
    ...
    scope_sha = scope_sha or _head(root)
    ...
    g0 = bool(HEX40.fullmatch(scope_sha)) and total == len(status_rows)
    gates.append(_gate(
        "G0_SCOPE_FREEZE",
        PASS if g0 else FAIL,
        "Exact commit and registry denominator are frozen." if g0 else "Scope is not frozen.",
        {"scope_sha": scope_sha, "registry_total": total, "status_rows": len(status_rows), "digest": digest},
        [] if g0 else ["invalid_scope_or_denominator"],
    ))
    ...
    gates.append(_gate(
        ...
        "Current source-control surfaces reconcile." if g1 else "Control-plane truth is contradictory.",
        {
    ...
    if run_preflight:
        ...
        gates.append(_gate(
            ...
            "Strict preflight passed." if g2 else "Strict preflight found structural errors.",
            {
                ...
                "missing_key_source_ids": [x["source_id"] for x in pf.get("missing_keys", [])],
            },
    ...
    gates.append(_gate(
        ...
        "All required sources are fully materialized." if g3 else "Required-source residue remains.",
        {
            ...
            "required_blockers": [
                {
                    ...
                    "expected_outputs": [x for x in row["expected_outputs"].split(";") if x],
    ...
    invalid_states = sorted({row["pipeline_status"] for row in status_rows if row["pipeline_status"] not in allowed})
    ...
    gates.append(_gate(
        ...
        "Every source has one recognized state." if g4 else "Unknown or duplicate source state exists.",
        {"pipeline_status_counts": dict(sorted(status_counts.items())), "invalid_states": invalid_states},
        invalid_states,
    ))
    ...
    gates.append(_gate(
        ...
        "All automatable sources are materialized." if g5 else "Automatable execution is incomplete.",
        {
    ...
    gates.append(_gate(
        ...
        "Every in-scope source meets a coverage contract." if g6 else "Coverage validation remains incomplete.",
        {
        ...
        [k for k in ("below_contract", "unverifiable", "uncontracted") if coverage_status.get(k, 0)],
    ))
    ...
    gates.append(_gate(
        ...
        "No unresolved entity-review residue." if g7 else "Entity review residue remains.",
        {
            ...
            "issue_types": dict(sorted(Counter(row["issue_type"] for row in open_reviews).items())),
        },
    ...
    gates.append(_gate(
        ...
        "Current-denominator audit proves zero orphan rows." if g8 else "Coverage/lineage audit is stale for current denominator.",
        {
            "coverage_audit_total_sources": coverage_total,
            "current_registry_total_sources": total,
            "coverage_audit_orphan_rows": orphan_rows,
            "historical_unresolved_lineage_rows": historical_status.get("materialization_coverage", {})
    ...
    gates.append(_gate(
        ...
        "Canonical master invariant receipt is certified." if g9 else "Canonical master remains diagnostic.",
        {
    ...
    gates.append(_gate(
        ...
        "All automatable sources are explicitly fresh." if g10 else "Freshness/universe evidence is incomplete.",
        {
    ...
    gates.append(_gate(
        ...
        "Federation production export is certified." if g11 else "Federation manifest blocks production/live execution.",
        {
    ...
    activation = bool(historical_status.get("preservation", {}).get("production_activation_authorized"))
    g12 = not upstream_nonpass and activation
    gates.append(_gate(
        "G12_RELEASE_CERTIFICATION",
        PASS if g12 else BLOCKED,
        "All gates pass and activation is authorized." if g12 else "Release certification remains blocked.",
        {"upstream_nonpass_gates": upstream_nonpass, "production_activation_authorized": activation},
    ...
    return {
        ...
        "gates": gates,
        "certification_state": config["states"]["certified"] if all_pass else config["states"]["non_production"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fail-closed moneysweep-pr production certification audit.")
    parser.add_argument("--root", type=Path, default=ROOT)
    parser.add_argument("--config", type=Path, default=DEFAULT_CONFIG)
    parser.add_argument("--scope-sha")
    parser.add_argument("--run-preflight", action="store_true")
    parser.add_argument("--output", type=Path, default=ROOT / "reports" / "production_certification.json")

...
def build(root: Path, *, operator_corpus_authoritative: bool) -> dict[str, Any]:
    ...
    required_total = 0
    required_full = 0
    full = partial = missing = no_outputs = 0
    ...
    total_rows = accounted_rows = orphan_rows = 0
    orphan_files = 0
    if processed.exists():
    ...
    return {
        ...
        "audit_scope": {
            ...
            "corpus_class": "authoritative_operator_corpus"
            if operator_corpus_authoritative
            else "checkout_visible_files_only",
            "warning": None
            if operator_corpus_authoritative
            else (
                "This audit measures only files visible in this checkout. It must not be "
                "used to certify operator-corpus completeness or provenance closure."
            ),
        },
        "local_truth_summary": {
        ...
        "processed_file_inventory": {
            ...
            "orphan_file_count": orphan_files,
            "files": inventory,
        },
        "sources": source_rows,
    }
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "operator_corpus_authoritative": report["audit_scope"]["operator_corpus_authoritative"],
                "orphan_rows": report["processed_file_inventory"]["orphan_rows"],
                "orphan_file_count": report["processed_file_inventory"]["orphan_file_count"],
            },
            sort_keys=True,
        )
    )

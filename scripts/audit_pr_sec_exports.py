...
from moneysweep.query.entity_types import SUPPORTED_KINDS, EntityIdentifier, EntityQuery
...
def test_supported_kinds_matches_literal():
    expected = {"uei", "name", "cage", "duns", "ein", "cik", "cusip"}
...
@pytest.mark.unit
def test_cusip_canonicalization_uppercases_and_dedups():
    eq = EntityQuery(
        identifiers=(
            EntityIdentifier(kind="cusip", value="30040p103"),
            EntityIdentifier(kind="cusip", value="30040P103"),
        )
    )
    assert eq.canonical_dict()["identifiers"] == [
        {"kind": "cusip", "value": "30040P103"}
    ]
    assert eq.by_kind("cusip") == ["30040P103"]

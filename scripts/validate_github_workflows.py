#!/usr/bin/env python3
"""Static checks for GitHub workflow syntax and repository safety invariants."""
...
def validate_workflow_file(path: Path) -> list[str]:
    ...
    if path.name in LIVE_FETCH_WORKFLOWS:
        ...
        if not isinstance(jobs, dict) or "validate_dispatch" not in jobs:
            errors.append(f"{path}: missing validate_dispatch job")

        for node_path, value in _walk(jobs):
            if node_path and node_path[-1] == "run" and isinstance(value, str):
                if "validate_live_fetch_dispatch.py" in value:
                    break
        else:

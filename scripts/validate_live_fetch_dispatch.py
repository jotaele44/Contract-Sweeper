#!/usr/bin/env python3
"""Validate operator-controlled live-fetch workflow dispatch inputs.

This module is intentionally stdlib-only so the preflight job can run before
installing project dependencies or touching the network.
"""
...
class DispatchValidationError(ValueError):
    """Raised when a dispatch would violate the live-fetch safety contract."""
    ...
    *,
    ...
    if WORKFLOWS[workflow]["days"]:
        ...
        try:
            ...
            raise DispatchValidationError("days must be an integer") from exc
        if not 1 <= parsed_days <= 1095:
            raise DispatchValidationError("days must be between 1 and 1095")
    elif days not in (None, ""):
        raise DispatchValidationError(f"{workflow} does not accept days")

    if mode == "fetch":
        if confirm != "YES":
            raise DispatchValidationError("live fetch requires confirm=YES")
        if WORKFLOWS[workflow]["secondary"] and confirm_secondary != "FETCH":
            raise DispatchValidationError("live fetch requires confirm_secondary=FETCH")

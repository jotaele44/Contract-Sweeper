...
def download_window(session, window, raw_dir, force, logger):
    ...
    for subtier in SUBTIER_AGENCIES:
        ...
        for filter_type in ("pop", "recipient"):
            ...
            fpath = raw_dir / fname
            payload = _build_payload(filter_type, window, subtier)
...
def build_master(raw_dir, master_path, logger):
    files = sorted(
        f for f in raw_dir.glob("hhs_*.csv") if f.with_name(f"{f.name}.meta.json").exists()
    )
    if not files:
...
def _run(root=None, force=False, fy_start=None):
    ...
    if all_errors:
        logger.error("  One or more requests failed; preserving the previous HHS master")
        master_rows = (

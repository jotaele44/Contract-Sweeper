...
def run(root: Path = Path(".")) -> dict[str, int]:
    ...
    graph = build_political_finance_graph(
        ...
        committees=inputs["committees"],
        disbursements=inputs["disbursements"],
        independent_expenditures=inputs["independent_expenditures"],
        oce_donations=inputs["oce_donations"],
        cee_donations=inputs["cee_donations"],
        entity_frames=entity_frames,
    )
    affiliation_edges = build_affiliation_edges(inputs["candidates"], inputs["committees"])
    committee_ids = (
        inputs["committees"].get("committee_id", pd.Series(dtype=str)).dropna().astype(str)
    )
    transfer_edges = build_transfer_edges(inputs["disbursements"], committee_ids)
    graph["edges"] = pd.concat(
        [graph["edges"], affiliation_edges, transfer_edges], ignore_index=True
    ).drop_duplicates("edge_id")

    temporal = correlate_temporal_activity(
        graph["edges"], graph["entities"], inputs["awards"], inputs["lobbying"]
    )
    graph["correlations"] = pd.concat(
        [graph["correlations"], temporal], ignore_index=True
    ).drop_duplicates()
    ...
    flow_paths = (
        ...
        if paths
        else pd.DataFrame(columns=["origin_entity_id", "terminal_entity_id", "hop_count", "path"])
    )
    ...
    accounting.to_csv(
        out / "political_materialization_accounting.csv", index=False, encoding="utf-8"
    )

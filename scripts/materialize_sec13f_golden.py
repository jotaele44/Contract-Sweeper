...
from moneysweep.runtime.source_registry import REPO_ROOT, all_sources, source_by_id

from .adapters import ADAPTER_REGISTRY, ENTITY_ADAPTER_REGISTRY, get_adapter, get_entity_adapter
from .cache import FileCache, ttl_for_cadence
from .entity_types import EntityQuery
from .types import CredentialMissing, ManualOnlyError, Query, QueryResult, SourceQueryOutcome
...
def _query_one(
    ...
    *,
    ...
    if not force_refresh:
        ...
        if hit is not None:
            df, meta = hit
            return SourceQueryOutcome(source_id=source_id, status="cache_hit", df=df, rows=int(len(df)), fetched_at=meta.get("fetched_at"))

    adapter = get_adapter(source_id, root=root)
    try:
        raw = adapter.fetch(criteria)
    except ManualOnlyError as exc:
        return SourceQueryOutcome(source_id=source_id, status="manual_only", df=None, rows=0, fetched_at=None, reason=str(exc))
    except CredentialMissing as exc:
        return SourceQueryOutcome(source_id=source_id, status="error", df=None, rows=0, fetched_at=None, error=str(exc))
    except Exception as exc:  # noqa: BLE001
        _LOG.exception("adapter %s failed", source_id)
        return SourceQueryOutcome(source_id=source_id, status="error", df=None, rows=0, fetched_at=None, error=f"{type(exc).__name__}: {exc}")
    ...
    return SourceQueryOutcome(source_id=source_id, status="ok", df=enriched, rows=int(len(enriched)), fetched_at=_utcnow_iso())
...
def query(
    ...
    *,
    ...
    for sid in source_ids:
        result.outcomes[sid] = _query_one(sid, criteria, root=root, cache=cache, force_refresh=force_refresh)
    return result
...
def _entity_query_one(
    ...
    *,
    ...
    return SourceQueryOutcome(source_id=source_id, status="ok", df=raw, rows=int(len(raw)), fetched_at=_utcnow_iso())
...
def query_entities(
    ...
    *,
    ...
    """Run an on-demand entity-mode query across registered entity adapters."""
    root = Path(root) if root else REPO_ROOT
    cache = FileCache(root)
    if source_ids is None:
    ...
    result = QueryResult(query=criteria)
    for sid in source_ids:
        result.outcomes[sid] = _entity_query_one(sid, criteria, root=root, cache=cache, force_refresh=force_refresh)
    return result

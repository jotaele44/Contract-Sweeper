...
def build_master(raw_dir, master_path, logger):
    files = sorted(
        f for f in raw_dir.glob("doj_*.csv") if f.with_name(f"{f.name}.meta.json").exists()
    )
    if not files:
...
def _run(root=None, force=False, fy_start=None):
    ...
    if all_errors:
        logger.error("  One or more requests failed; preserving the previous DOJ master")
        master_rows = (

...
def main_with_args(argv: Sequence[str] | None = None) -> int:
    ...
    # spiderweb-pr's spatial lane (spiderweb-pr/readiness/spiderweb_spatial_lane.py)
    # consumes this file as its router-dropzone input, validated against
    # spiderweb-pr/schemas/pr_intake_derivative.schema.json. The writer was retired
    # in 2026-06 while the lane had no consumer; the lane now exists, so the
    # delivery stream is revived (federation handoff, 2026-07).

...
from moneysweep.case_manager import (
    AuditEvent, Case, CaseEvidence, Claim, ClaimEvidence, Contradiction, Finding,
    ValidationError, deterministic_id, validate_append_only_events, validate_case_bundle,
)
...
def test_claim_and_finding_are_separate_and_evidence_relations_are_explicit():
    ...
    claim = Claim(claim_id, case_id, "The projected completion date changed.", "schedule", "pending", .85, "linked")
    bundle = {
        "case": case,
        "canonical_evidence_ids": (evidence_id,),
        "case_evidence": (CaseEvidence(deterministic_id("case_evidence", case_id, evidence_id), case_id, evidence_id, "qpr"),),
        "claims": (claim,),
        "claim_evidence": (ClaimEvidence(deterministic_id("claim_evidence", claim_id, evidence_id, "support"), claim_id, evidence_id, "support"),),
        "contradictions": (),
        "findings": (Finding(deterministic_id("finding", claim_id), case_id, claim_id, "Schedule changed across reports.", .85, "reviewer", True, "accepted"),),
        "audit_events": (),
    }
...
def test_unresolved_contradictions_are_not_auto_collapsed():
    ...
    contradiction = Contradiction(deterministic_id("contradiction", c1, c2), case_id, (c1, c2), "temporal", "high")
...
def test_accepted_finding_requires_contradiction_review():
    ...
    bundle = {
        ...
        "canonical_evidence_ids": (), "case_evidence": (),
        "claims": (Claim(claim_id, case_id, "X", "test", "pending", .5, "inferred"),),
        "claim_evidence": (), "contradictions": (),
        "findings": (Finding(deterministic_id("finding", claim_id), case_id, claim_id, "X", .5, "r", False, "accepted"),),
        "audit_events": (),
    }
...
def test_audit_events_are_append_only_and_hash_chained():
    ...
    events = (
        AuditEvent(deterministic_id("audit_event", 1), case_id, 1, "2026-01-01T00:00:00Z", "tester", "create", "case", case_id, first_hash),
        AuditEvent(deterministic_id("audit_event", 2), case_id, 2, "2026-01-02T00:00:00Z", "tester", "review", "case", case_id, hashlib.sha256(b"second").hexdigest(), first_hash),
    )
...
def test_sql_migration_blocks_audit_event_update_and_delete():
    ...
    db.execute("INSERT INTO cases(case_id,title,case_type,status,scope,visibility) VALUES('case_x','X','audit','open','scope','internal')")
    db.execute("INSERT INTO case_audit_events VALUES('audit_event_x','case_x',1,'2026-01-01','tester','create','case','case_x',?,NULL)", ("a" * 64,))

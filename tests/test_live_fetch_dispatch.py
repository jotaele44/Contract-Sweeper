...
@pytest.mark.parametrize("workflow", ["highergov-fetch", "sam-opportunities-fetch"])
def test_fetch_requires_exact_confirmation_tokens(workflow: str) -> None:
    decision = validate_dispatch(
        workflow=workflow,
        mode="fetch",
        confirm="YES",
        confirm_secondary="FETCH",
...
@pytest.mark.parametrize("days", ["0", "3651", "abc", "18446744073709551617", "00001"])
def test_sam_days_are_bounded_without_fixed_width_arithmetic(days: str) -> None:
    with pytest.raises(DispatchValidationError):
        validate_dispatch(
            workflow="sam-opportunities-fetch",
            mode="preflight",

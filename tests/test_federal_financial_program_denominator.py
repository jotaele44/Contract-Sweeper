...
def test_current_sam_program_denominator_is_frozen_and_unique():
    ...
    assert (
        snap["source"]["sha256"]
        == "2daa2ac413acd04ade6d26d9a127e7519bdd721a803984ccf90c44ade19476a3"
    )
...
def test_financial_denominator_partitions_complete_assistance_denominator():
    ...
    assert (
        d["financial_program_rows"] + d["nonfinancial_assistance_rows"]
        == d["assistance_listing_rows"]
    )
...
def test_pr_nexus_baseline_is_100_percent_classified_but_not_promoted():
    ...
    assert (
        snap["initial_pr_nexus_policy"]["eligibility_alone_never_promotes_confirmed_activity"]
        is True
    )
...
def test_fain_backfill_is_blocked_until_program_population_gate_closes():
    ...
    snap = _json(SNAPSHOT)
    assert (
        r["bridge_to_existing_contract"]["global_fain_backfill_gate"]
        == "blocked_until_ontology_certified"
    )

...
def test_point_requires_real_coordinates():
    assert (
        point_feature(
            {"municipality": "San Juan"}, feature_id="x", feature_class="project", source_id="src"
        )
        is None
    )
...
def test_point_materializes_source_reported_geometry():
    f = point_feature(
        {"geo_lat": 18.45, "geo_lon": -66.1, "geo_attribution_confidence": "exact_name"},
        feature_id="p1",
        feature_class="project",
        source_id="src",
    )

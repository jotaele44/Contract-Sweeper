...
def test_namespace_occupancy_preserves_ddec_cfi_separation() -> None:
    ...
    assert registry.register(
        NamespaceBinding("CFI", "2020-000368", "Aggregate", "CFI-row-368")
    ).state is CertificationState.PASS
    assert registry.register(
        NamespaceBinding("CFI", "2020-000358", "Starbright", "CFI-row-358")
    ).state is CertificationState.PASS
    ...
    assert registry.register(
        NamespaceBinding("DDEC", "2020-000358", "FIE0057", "DDEC-FIE0057")
    ).state is CertificationState.PASS
...
def test_proximity_only_is_candidate_not_identity() -> None:
    result = resolve_candidates(
        [Candidate("parcel-a", EvidenceBasis.PROXIMITY_ONLY, "nearest")]
    )
...
def test_unlock_graph_requires_exact_prerequisites() -> None:
    assert parcel_unlock(
        {"authoritative_property_anchor": CertificationState.OPEN}
    ).state is CertificationState.BLOCKED
    assert federation_unlock(
        {"stable_id_bridge": CertificationState.PASS}
    ).state is CertificationState.PASS
    assert funding_unlock(
        {"project_specific_binding": CertificationState.PROVISIONAL}
    ).state is CertificationState.BLOCKED

...
resumable = load_module("materialize_ocpr_contracts_resumable", "scripts/materialize_ocpr_contracts_resumable.py")


def taxonomy():
    return yaml.safe_load((ROOT / "registries/om_contract_taxonomy.yaml").read_text(encoding="utf-8"))


def test_classifier_positive_review_and_negative_override():
    positive = pd.Series({"service_description": "Operation and preventive maintenance of water treatment plant", "contract_type": "services", "source_file": "x", "agency": "AAA"})
    review = pd.Series({"service_description": "Software maintenance", "contract_type": "services", "source_file": "x", "agency": "agency"})
    negative = pd.Series({"service_description": "Design-build new construction with maintenance manuals", "contract_type": "construction", "source_file": "x", "agency": "agency"})
...
def test_fingerprint_is_deterministic():
    row = pd.Series({"contract_number": "A-1", "contractor_name": "Vendor", "agency": "Agency", "contract_amount": "$10.00", "start_date": "2026-01-01", "service_description": "Maintenance"})
...
def test_checkpoint_rejects_tampered_work_file(tmp_path):
    ...
    checkpoint.update(written_rows=1, work_sha256=hashlib.sha256(work_path.read_bytes()).hexdigest())
...
def test_builder_emits_blockers_and_validator_passes_structure(tmp_path):
    ...
    (root / "registries/om_contract_taxonomy.yaml").write_text((ROOT / "registries/om_contract_taxonomy.yaml").read_text(encoding="utf-8"), encoding="utf-8")
    source = root / "data/staging/processed/pr_ocpr_contracts.csv"
    source.parent.mkdir(parents=True)
    pd.DataFrame([{"contract_number": "A-1", "contractor_name": "Vendor", "agency": "Agency", "contract_amount": "100", "start_date": "2026-01-01", "end_date": "2027-01-01", "service_description": "operation and preventive maintenance", "contract_type": "services"}]).to_csv(source, index=False)

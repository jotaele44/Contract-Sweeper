...
def test_contribution_endpoint_unifies_fec_and_oce(tmp_path, monkeypatch):
    pd.DataFrame([
        {
            "contributor_name": "DONOR A", "contribution_receipt_amount": "100",
            "contribution_receipt_date": "2024-01-01", "committee_name": "PAC A",
            "cycle": "2024", "is_individual": "True", "committee_id": "C1", "candidate_id": "",
        }
    ]).to_csv(tmp_path / "pr_fec_contributions.csv", index=False)
    pd.DataFrame([
        {
            "donor_name": "DONOR B", "amount": "200", "contribution_date": "2024-02-01",
            "candidate_or_committee": "COMITE B", "party": "PIP", "cycle": "2024",
        }

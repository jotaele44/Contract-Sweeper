...
def test_authorized_and_affiliated_edges():
    candidates = pd.DataFrame([{"candidate_id": "H1", "principal_campaign_committee_id": "C1", "record_id": "R1"}])
    committees = pd.DataFrame([{"committee_id": "C1", "affiliated_committee_id": "C2", "record_id": "R2"}])
...
def test_schedule_b_transfer_requires_known_committee():
    disbursements = pd.DataFrame([
        {"committee_id": "C1", "recipient_committee_id": "C2", "transaction_id": "T1", "disbursement_amount": "50"},
...
def test_temporal_contract_correlation_is_not_causal():
    ...
    edges = pd.DataFrame([{"source_entity_id": "donor_1", "target_entity_id": "fec_committee:C1", "transaction_date": "2025-01-01"}])
    awards = pd.DataFrame([{"awardee_id": "A1", "awardee_name": "ACME INC", "award_date": "2025-06-01"}])
...
def test_accounted_zero_rows():
    accounting = materialization_accounting({"schedule_e": pd.DataFrame()}, {"edges": pd.DataFrame([{"x": 1}])})

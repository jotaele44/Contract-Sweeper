...
def generate_candidate(root: Path, version: str) -> Path:
    policy = (root / "config" / "forensics" / "manual_promotion_policy.yaml").read_text(
        encoding="utf-8"
    )
    if "allow_automatic_promotion: false" not in policy:
        raise RuntimeError("Manual promotion policy is not enforced")
    queue = json.loads(
        (root / "reports" / "forensics" / "skill_improvement_queue.json").read_text(
            encoding="utf-8"
        )
    )
    approved = [p for p in queue if p.get("status") in {"TRIAGED", "APPROVED"}]
    target = root / "reports" / "forensics" / "candidates" / version

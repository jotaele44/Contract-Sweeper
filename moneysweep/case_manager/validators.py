...
def validate_append_only_events(events) -> None:
    """Require strictly increasing, unique audit sequences and a complete hash chain."""
    seen_ids: set[str] = set()
    previous = None
    for expected, event in enumerate(sorted(events, key=lambda e: e.sequence), start=1):
        ...
        if expected > 1 and not event.previous_event_sha256:
            raise ValidationError("later audit events must reference the prior event hash")
...
def validate_case_bundle(bundle: dict) -> None:
    ...
    claim_ids = {x.claim_id for x in bundle.get("claims", ())}

    for link in bundle.get("case_evidence", ()):
        if link.case_id != case_id or link.evidence_id not in evidence_ids:
            raise ValidationError("case evidence must reference this case and an existing canonical evidence ID")
    ...
    for finding in bundle.get("findings", ()):
        if finding.claim_id not in claim_ids:
            raise ValidationError("finding must reference a distinct claim")

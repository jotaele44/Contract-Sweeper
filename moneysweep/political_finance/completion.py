...
def build_affiliation_edges(candidates: pd.DataFrame, committees: pd.DataFrame) -> pd.DataFrame:
    ...
    if candidates is not None and not candidates.empty:
        for i, row in enumerate(candidates.fillna("").to_dict("records")):
            ...
            rows.append(
                _edge(source, target, "AUTHORIZED_COMMITTEE_OF", "fec_candidates", record_id, row)
            )
    if committees is not None and not committees.empty:
        for row in committees.fillna("").to_dict("records"):
            ...
            rows.append(
                _edge(
                    f"fec_committee:{source_id}",
                    f"fec_committee:{target_id}",
                    "AFFILIATED_WITH",
                    "fec_committees",
                    record_id,
                    row,
...
def correlate_temporal_activity(
    ...
    names = {
        str(r.entity_id): normalize_name(r.canonical_name)
        for r in entities[["entity_id", "canonical_name"]].itertuples(index=False)
    }
    activity = political_edges.copy()
    activity["transaction_date"] = pd.to_datetime(
        activity["transaction_date"], errors="coerce", utc=True
    )
    result: list[dict] = []
    specs = [
        (
            "contracts",
            awards,
            ("awardee_id", "recipient_id", "entity_id"),
            ("awardee_name", "recipient_name", "canonical_name"),
            ("award_date", "start_date", "action_date"),
            "POLITICAL_ACTIVITY_PRECEDES_AWARD",
        ),
        (
            "lobbying",
            lobbying,
            ("client_id", "entity_id", "registrant_id"),
            ("client_name", "name", "canonical_name"),
            ("filing_date", "period_end", "date"),
            "POLITICAL_ACTIVITY_NEAR_LOBBYING",
        ),
    ]
    for dataset, frame, id_candidates, name_candidates, date_candidates, relation in specs:
        ...
        ext["_date"] = (
            pd.to_datetime(ext[date_col], errors="coerce", utc=True) if date_col else pd.NaT
        )
        for entity_id, canonical in names.items():
            ...
            if matched.empty:
                continue
            edge_dates = activity.loc[
                (activity["source_entity_id"] == entity_id)
                | (activity["target_entity_id"] == entity_id),
                "transaction_date",
            ].dropna()
            for row in matched.to_dict("records"):
                ...
                result.append(
                    {
                        "political_entity_id": entity_id,
                        "external_entity_id": f"{dataset}:{row[id_col]}",
                        "external_dataset": dataset,
                        "relationship_type": relation,
                        "match_method": "exact_normalized_name+temporal"
                        if temporal
    ...
    return (
        pd.DataFrame(result, columns=columns).drop_duplicates()
        if result
        else pd.DataFrame(columns=columns)
    )


def materialization_accounting(
    inputs: dict[str, pd.DataFrame], outputs: dict[str, pd.DataFrame]
) -> pd.DataFrame:
            ...
            rows.append(
                {
                    ...
                    "dataset": name,
                    "row_count": count,
                    "status": "nonzero" if count else "accounted_zero",
                    "reason": "materialized"
                    if count
                    else "source absent, filtered empty, or no qualifying records",
                }
            )
...
def _edge(
    ...
    return {
        "edge_id": _id("pfe", source_id, record_id, source, target, edge_type),
        ...
        "confidence": confidence,
        "provenance": f"{source_id}:{record_id}",
    }


def _dedup(rows: list[dict]) -> pd.DataFrame:
    return (
        pd.DataFrame(rows, columns=EDGE_COLUMNS).drop_duplicates("edge_id")
        if rows
